...
# Author: Wang
# Extracted and merged from MediaCrawler playwright_sign.py + xhs_sign.py
# License: Non-Commercial Learning Use Only
#
# Generates XHS API request signatures via Playwright injection of window.mnsv2
...
def mrc(e: str) -> int:
    """CRC32 variant for x-s-common x9 field"""
    o = -1
...
def encode_utf8(s: str) -> list:
    """Encode string to UTF-8 byte list via URL encoding"""
...
def b64_encode(data: list) -> str:
    """Custom Base64 encoding with XHS shuffled character table"""
    length = len(data)
    remainder = length % 3
    chunks = []
...
def get_trace_id() -> str:
    """Generate random trace id for X-B3-Traceid header"""
...
async def sign_request(
    ...
    Args:
        page: Playwright Page with XHS loaded (window.mnsv2 available)
    ...
    Returns:
        Dict with x-s, x-t, x-s-common, x-b3-traceid
    """
